...
namespace Zero.Core.Web
{
    public class HttpHelper
    {
        ...
        {
            HttpCookie cookie = new HttpCookie("zero");
            cookie.Values["id"] = "55";
            cookie.Values["name"] = "xiao";
        ...
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies["zero"];
            if (cookie == null)
            {
                cookie.Values.Remove("name"); //移除值
                cookie.Values.Set("id", "5");
                return string.Empty;
            }
            return cookie.Values["id"];
        }
    }
}
