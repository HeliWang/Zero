...
lxb.add("config", function (a) {
    var c = lxb.use("base");
    var e = {
        BDCBID: 'adccb335-6a41-47f4-a906-7d737ece40f9',
        LXBVT: 0,
        TEMPSITEID: '2022093',
...
lxb.add("business.pin", function (g) {
    ...
    var l = lxb.use("config").Lang;
    var k = lxb.use("config").ClassName;
    var a = lxb.use("config").SiteId;
    var i = lxb.use("util");
    var h = {};
    var o = {};
    var m = "http://lxbjs.baidu.com/cb/scode?t=";

    function b() {
        var r = o.main = c.create("ins", {
            className: k.MAIN + "-pin-con"
        });
        o.main.innerHTML = d.CB_PIN;
        var q = c.q("lxbPinItem", o.main);
        o.btnClose = q[0];
        o.input = q[1];
        o.pinImg = q[2];
        o.change = q[3];
        ...
        o.btnOk = q[5];
        o.btnCancle = q[6];
        var s = c.viewportSize();
        var p = parseInt(h.main.style.top, 10) + 124;
        if (p >= s.height) {
            o.main.style.top = -(p - s.height) + "px"
        }
        h.main.appendChild(o.main);
    ...
    function f() {
        ...
        o.btnClose.onclick = o.btnCancle.onclick = function () {
            j()
        };
        o.input.onkeyup = function (q) {
            q = q || window.event;
            if (q.keyCode == 13) {
                var p = o.input.value;
                if (!n(p)) {
        ...
        o.change.onclick = function () {
            o.pinImg.src = m + new Date().getTime()
        }
    }
    function n(p) {
        p = c.trim(p);
        if (p == "") {
            o.tip.innerHTML = l.CB_PIN_NULL;
            return false
        }
        if (p.length != 4) {
            o.tip.innerHTML = l.CB_PIN_WRONG;
            return false
        }
        return true
    }
    function e(r) {
        var s = lxb.use("net");
        var t = lxb.use("business.callback");
        var q = "http://lxbjs.baidu.com/cb/callcode";
        var p = lxb.use("config").bdcbid;
        s.send(q, {
            scode: r
        }, function (u) {
            if (!!u.status) {
                o.tip.innerHTML = u.statusInfo || l.CB_PIN_WRONG;
                return
            } else {
                j();
                t.submit(h.value, h.groupId)
            }
        })
    }
    function j() {
...
lxb.add("business.callback", function (q) {
    var i = lxb.use("base");
    var h = lxb.use("config").Lang;
    var d = lxb.use("config").TPL;
    var f = lxb.use("config").ClassName;
    ...
    function r() {
        ...
        var y = k.cbCon = i.create("ins", {
            className: "lxb-callback-container"
        });
        k.cbCon.appendChild(t);
        var v = k.btn = i.create("ins", {
            className: f.CB_INPUT_BTN
        });
        v.innerHTML = i.encodeHTML(g.btnFontContent || "");
        if (!(b.isStandard() && b.isHorizon())) {
            v.style.color = g.btnfontColor;
            v.style.backgroundColor = g.btnColor
        }
        k.cbCon.appendChild(v);
        if (b.displayGroup()) {
            v.style.display = "none";
            var x = k.groupContainer = i.create("ins", {
                className: "lxb-group-container"
            });
            k.cbCon.appendChild(x);
            for (var u = 0; u < g.groupDetail.length; u++) {
                var z = i.create("ins", {
                    groupid: g.groupDetail[u].groupid,
                    title: "\u514D\u8D39\u56DE\u7535",
                    className: "lxb-group-btn"
                });
                z.innerHTML = i.encodeHTML(g.groupDetail[u].groupname);
                z.style.color = g.btnfontColor;
                z.style.backgroundColor = g.btnColor;
                x.appendChild(z)
            }
        }
        var w = k.tip = i.create("ins", {
            className: f.CB_INPUT_TIP
        });
        w.style.display = "none";
        w.innerHTML = d.CB_INPUT_TIP_1;
        if (i.ie && i.ie <= 6) {
            w.appendChild(i.create("iframe", {
                className: f.MAIN + "-mask",
                frameBorder: 0
            }))
        }
        g.main.appendChild(y);
        g.main.appendChild(w);
        if (b.isPin()) {
...
lxb.add("business.invite", function (d) {
    ...
    d.init = function (n) {
        ...
        var r = a.q("lxb-cb-input")[0];
        if (!r) {
            return
        }
        if (f.displayGroup()) {
            var s = a.q("lxb-group-container")[0]
        } else {
            var q = a.q("lxb-cb-input-btn")[0]
        }
        if (f.isPin()) {
            var l = a.q("lxb-cb-pin-input")[0]
        }
        var o = function () {
...
(function () {
    ...
    function d(x) {
        ...
        if (u.base) {
            ...
            w.loadCSS(s);
            y = lxb.use("business.container").init(u.base, function (z) {
                if (u.custom) {
